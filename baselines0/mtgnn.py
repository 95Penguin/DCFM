...
修复:
  [Bug2]  GraphLearner 补回反对称差 + topk 稀疏化
  [Bug3]  mixprop 数据格式 [B,C,N,T]，cat 所有阶后统一线性映射
  [Bug4]  dilated_inception 对齐官方，不做 same-padding
  [Bug5]  前向图用 adp，后向图用 adp.T
  [Bug13] Norm 改用官方自定义 LayerNorm
  [新Bug] skip_convs 改为 (1,1) kernel，forward 中统一取最后时间步
          避免 kernel size 与实际时间维不匹配导致 crash
"""
import torch
...
class GraphLearner(nn.Module):
    ...
    def forward(self, idx: torch.Tensor) -> torch.Tensor:
        ...
        s1, t1 = (adj + torch.rand_like(adj) * 0.01).topk(self.top_k, dim=1)
...
class MTGNN(nn.Module):
    """
    MTGNN 单步预测版本 (T_out=1)，对齐官方 gtnet 架构。
    ...
    def __init__(self,
        ...
        # [修复] skip_convs 统一用 (1,1) kernel，forward 中取最后时间步
        ...
        self.end_conv2 = nn.Conv2d(end_dim,  out_dim,  kernel_size=(1, 1))
    ...
    def forward(self, x: torch.Tensor,
        ...
        x_out = self.end_conv2(x_out)                # [B, out_dim, N, 1]
        return x_out.squeeze(-1).permute(0, 2, 1)    # [B, N, out_dim]
