...
def setup_logger(cfg_train, dataset_name: str = "") -> logging.Logger:
    ...
    if cfg_train.log_dir is not None:
        ...
        log_file  = os.path.join(
            cfg_train.log_dir,
            f"gridcfn_{dataset_name}_{timestamp}.log"
        )
...
def load_data(cfg: Config):
    d = cfg.data
    assert d.data_path, f"data_path 未设置，请在 config.py 或 --preset 中指定"
...
def build_model(cfg: Config) -> GridCFN:
    m = cfg.model
    return GridCFN(
        in_dim=m.in_dim,         gcn_hidden=m.gcn_hidden,
...
def main(cfg: Config):
    logger = setup_logger(cfg.train, cfg.data.dataset)
    ...
    # 2. [Fix-I] 预计算 adj_norm 和 edge_index（整个训练期间固定，只算一次）
    adj_norm   = GridCFN.normalize_adj(adj)
    edge_index = GridCFN.adj_to_edge_index(adj)
    logger.info(
        f"Graph  : {adj.shape[0]} nodes, {edge_index.shape[1]} edges"
    )
    ...
    history = train(
        ...
        adj_norm=adj_norm,       # [Fix-I] 传预计算结果
        edge_index=edge_index,   # [Fix-I] 传预计算结果
        device=device,
        scaler=scaler,
        cfg_train=cfg.train,
        logger=logger,
