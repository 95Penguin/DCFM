"""
baselines/csdi.py
CSDI: Conditional Score-based Diffusion Models for Probabilistic Time Series Imputation
论文: Tashiro et al., NeurIPS 2021  https://arxiv.org/abs/2107.03502
参考: https://github.com/ermongroup/CSDI

修复:
  [Bug10] DDPM 逆向后验方差用 beta_tilde = beta*(1-alpha_bar_{t-1})/(1-alpha_bar_t)
  [Bug11] 去噪网络输入在 channel 维 stack（noisy + mask），数据格式 [B,C,K,L]
  [新Bug] ResidualBlock 时序/特征注意力 reshape 维度顺序修正
          time: [B,C,K,L] → permute(0,2,3,1) → [B*K,L,C] → attention → reshape back
          feat: [B,C,K,L] → permute(0,3,2,1) → [B*L,K,C] → attention → reshape back
"""
import math
import torch
import torch.nn as nn
import torch.nn.functional as F


# ── 扩散时间步嵌入 ─────────────────────────────────────────────────────────

class DiffusionEmbedding(nn.Module):
    """sinusoidal embedding → 两层 Linear + SiLU（对齐官方）"""
    def __init__(self, num_steps: int, embedding_dim: int = 128,
                 projection_dim: int = None):
        super().__init__()
        projection_dim = projection_dim or embedding_dim
        self.register_buffer(
            'embedding',
            self._build_embedding(num_steps, embedding_dim // 2),
            persistent=False,
        )
        self.projection = nn.Sequential(
            nn.Linear(embedding_dim,  projection_dim),
            nn.SiLU(),
            nn.Linear(projection_dim, projection_dim),
        )

    @staticmethod
    def _build_embedding(num_steps: int, dim: int) -> torch.Tensor:
        steps = torch.arange(num_steps, dtype=torch.float32).unsqueeze(1)
        dims  = torch.arange(dim,       dtype=torch.float32).unsqueeze(0)
        table = steps * torch.exp(-math.log(10000) * dims / dim)
        return torch.cat([torch.sin(table), torch.cos(table)], dim=1)

    def forward(self, diffusion_step: torch.Tensor) -> torch.Tensor:
        return self.projection(self.embedding[diffusion_step])


# ── ResidualBlock ─────────────────────────────────────────────────────────

class ResidualBlock(nn.Module):
    """
    官方 ResidualBlock，数据格式 [B, C, K, L]。
    时序注意力（跨 L）: [B*K, L, C]
    特征注意力（跨 K）: [B*L, K, C]
    [新Bug 修复] reshape/permute 顺序全部对齐。
    """
    def __init__(self, side_dim: int, channels: int,
                 diffusion_embedding_dim: int, nheads: int):
        super().__init__()
        self.diffusion_projection = nn.Linear(diffusion_embedding_dim, channels)
        self.cond_projection      = nn.Conv2d(side_dim, 2 * channels, kernel_size=1)
        self.mid_projection       = nn.Conv2d(channels, 2 * channels, kernel_size=1)
        self.time_layer    = nn.MultiheadAttention(channels, nheads, batch_first=True)
        self.feature_layer = nn.MultiheadAttention(channels, nheads, batch_first=True)
        self.output_projection = nn.Conv2d(channels, 2 * channels, kernel_size=1)

    def forward(self, x: torch.Tensor,
                cond_info: torch.Tensor,
                diffusion_emb: torch.Tensor) -> tuple:
        """
        x             : [B, C, K, L]
        cond_info     : [B, side_dim, K, L]
        diffusion_emb : [B, diffusion_embedding_dim]
        returns       : ([B, C, K, L], [B, C, K, L])
        """
        B, C, K, L = x.shape

        # 扩散时步条件
        diff_proj = self.diffusion_projection(diffusion_emb)       # [B, C]
        y = x + diff_proj.unsqueeze(-1).unsqueeze(-1)              # [B, C, K, L]
        y = self.mid_projection(y)                                 # [B, 2C, K, L]

        # 时序注意力（跨 L 维）
        # [B, C, K, L] → permute(0,2,3,1) → [B, K, L, C] → reshape → [B*K, L, C]
        y_time = y[:, :C].permute(0, 2, 3, 1).reshape(B * K, L, C)
        y_time, _ = self.time_layer(y_time, y_time, y_time)        # [B*K, L, C]
        y_time = y_time.reshape(B, K, L, C).permute(0, 3, 1, 2)   # [B, C, K, L]

        # 特征注意力（跨 K 维）
        # [B, C, K, L] → permute(0,3,2,1) → [B, L, K, C] → reshape → [B*L, K, C]
        y_feat = y[:, C:].permute(0, 3, 2, 1).reshape(B * L, K, C)
        y_feat, _ = self.feature_layer(y_feat, y_feat, y_feat)     # [B*L, K, C]
        y_feat = y_feat.reshape(B, L, K, C).permute(0, 3, 2, 1)   # [B, C, K, L]

        # 门控合并
        y = torch.tanh(y_time) * torch.sigmoid(y_feat)            # [B, C, K, L]

        # 条件注入
        cond_out = self.cond_projection(cond_info)                 # [B, 2C, K, L]
        y = y + cond_out[:, :C]

        # 输出投影
        residual, skip = self.output_projection(y).chunk(2, dim=1)
        return (x + residual) / math.sqrt(2.0), skip


# ── DDPM 调度器 ───────────────────────────────────────────────────────────

class DDPMScheduler:
    """
    标准 DDPM 线性 beta 调度。
    [Bug10 修复] 后验方差用 beta_tilde 而非 beta。
    """
    def __init__(self, n_steps: int = 100,
                 beta_start: float = 1e-4, beta_end: float = 0.02):
        self.n_steps = n_steps
        beta      = torch.linspace(beta_start, beta_end, n_steps)
        alpha     = 1.0 - beta
        alpha_bar = torch.cumprod(alpha, dim=0)
        alpha_bar_prev = torch.cat([torch.tensor([1.0]), alpha_bar[:-1]])
        # 后验方差
        beta_tilde = beta * (1 - alpha_bar_prev) / (1 - alpha_bar).clamp(min=1e-8)

        self.beta            = beta
        self.alpha           = alpha
        self.alpha_bar       = alpha_bar
        self.alpha_bar_prev  = alpha_bar_prev
        self.beta_tilde      = beta_tilde
        self.sqrt_alpha_bar            = alpha_bar.sqrt()
        self.sqrt_one_minus_alpha_bar  = (1 - alpha_bar).sqrt()
        self.sqrt_recip_alpha          = (1.0 / alpha).sqrt()

    def to(self, device: torch.device) -> 'DDPMScheduler':
        for attr in ['beta', 'alpha', 'alpha_bar', 'alpha_bar_prev',
                     'beta_tilde', 'sqrt_alpha_bar',
                     'sqrt_one_minus_alpha_bar', 'sqrt_recip_alpha']:
            setattr(self, attr, getattr(self, attr).to(device))
        return self

    def q_sample(self, x0: torch.Tensor, t: torch.Tensor,
                 noise: torch.Tensor) -> torch.Tensor:
        """前向加噪: x_t = sqrt(ā_t)*x0 + sqrt(1-ā_t)*noise"""
        sa  = self.sqrt_alpha_bar[t]
        soa = self.sqrt_one_minus_alpha_bar[t]
        for _ in range(x0.dim() - 1):
            sa  = sa.unsqueeze(-1)
            soa = soa.unsqueeze(-1)
        return sa * x0 + soa * noise

    def p_mean_variance(self, x_t: torch.Tensor, t: torch.Tensor,
                        eps_pred: torch.Tensor) -> tuple:
        """
        DDPM 逆向：后验均值 + 后验方差（beta_tilde）。
        """
        def _b(tensor):
            v = tensor[t]
            for _ in range(x_t.dim() - 1):
                v = v.unsqueeze(-1)
            return v

        mu = _b(self.sqrt_recip_alpha) * (
            x_t - _b(self.beta) / _b(self.sqrt_one_minus_alpha_bar) * eps_pred
        )
        var = _b(self.beta_tilde)
        return mu, var


# ── 去噪网络 diff_CSDI ────────────────────────────────────────────────────

class diff_CSDI(nn.Module):
    """
    官方 diff_CSDI 架构。
    输入: stack([x_noisy, cond_mask], dim=1) → [B, 2, K, L]
    输出: [B, K, L]（预测噪声）
    """
    def __init__(self,
                 num_nodes:    int,
                 T_total:      int,
                 channels:     int  = 64,
                 n_layers:     int  = 4,
                 nheads:       int  = 8,
                 diffusion_dim: int = 128,
                 side_dim:     int  = 128,
                 inputdim:     int  = 2):
        super().__init__()
        self.channels  = channels
        self.num_nodes = num_nodes
        self.T_total   = T_total

        self.diffusion_embedding = DiffusionEmbedding(
            num_steps=1000, embedding_dim=diffusion_dim)

        self.input_projection   = nn.Conv2d(inputdim,  channels, kernel_size=1)
        self.output_projection1 = nn.Conv2d(channels, channels,  kernel_size=1)
        self.output_projection2 = nn.Conv2d(channels, 1,         kernel_size=1)
        nn.init.zeros_(self.output_projection2.weight)

        # 节点 + 时间位置编码拼接作为 side_info
        self.node_emb = nn.Embedding(num_nodes, side_dim // 2)
        self.time_emb = nn.Embedding(T_total,   side_dim // 2)

        self.residual_layers = nn.ModuleList([
            ResidualBlock(
                side_dim               = side_dim,
                channels               = channels,
                diffusion_embedding_dim = diffusion_dim,
                nheads                 = nheads,
            )
            for _ in range(n_layers)
        ])

        self.register_buffer('node_idx', torch.arange(num_nodes))
        self.register_buffer('time_idx', torch.arange(T_total))

    def _build_side_info(self, B: int) -> torch.Tensor:
        """返回 [B, side_dim, K, L]"""
        n_emb = self.node_emb(self.node_idx)              # [K, D/2]
        n_emb = n_emb.T.unsqueeze(0).unsqueeze(-1)        # [1, D/2, K, 1]
        n_emb = n_emb.expand(B, -1, -1, self.T_total)    # [B, D/2, K, L]

        t_emb = self.time_emb(self.time_idx)              # [L, D/2]
        t_emb = t_emb.T.unsqueeze(0).unsqueeze(2)        # [1, D/2, 1, L]
        t_emb = t_emb.expand(B, -1, self.num_nodes, -1)  # [B, D/2, K, L]

        return torch.cat([n_emb, t_emb], dim=1)           # [B, side_dim, K, L]

    def forward(self, x_noisy: torch.Tensor,
                cond_mask: torch.Tensor,
                diffusion_step: torch.Tensor) -> torch.Tensor:
        """
        x_noisy      : [B, K, L]
        cond_mask    : [B, K, L]  1=已知, 0=待预测
        diffusion_step: [B] LongTensor
        returns      : [B, K, L]
        """
        B, K, L = x_noisy.shape

        x_inp = torch.stack([x_noisy, cond_mask], dim=1)  # [B, 2, K, L]
        h = F.relu(self.input_projection(x_inp))           # [B, C, K, L]

        diff_emb = self.diffusion_embedding(diffusion_step)  # [B, diff_dim]
        side     = self._build_side_info(B)                  # [B, side_dim, K, L]

        skip_sum = torch.zeros_like(h)
        for layer in self.residual_layers:
            h, skip = layer(h, side, diff_emb)
            skip_sum = skip_sum + skip

        out = skip_sum / math.sqrt(len(self.residual_layers))
        out = F.relu(self.output_projection1(out))         # [B, C, K, L]
        out = self.output_projection2(out)                 # [B, 1, K, L]
        return out.squeeze(1)                              # [B, K, L]


# ── CSDI 完整模型 ─────────────────────────────────────────────────────────

class CSDI(nn.Module):
    """
    CSDI 单步超前预测 (T_out=1)。
    训练: compute_loss(x, y) — DDPM 噪声预测 MSE
    推理: forward(x)  — n_samples 次 DDPM 逆向采样取均值
    """
    def __init__(self,
                 num_nodes:       int,
                 in_dim:          int,
                 T_in:            int = 168,
                 channels:        int = 64,
                 n_layers:        int = 4,
                 nheads:          int = 8,
                 diffusion_steps: int = 100,
                 n_samples:       int = 10,
                 out_dim:         int = 1):
        super().__init__()
        self.T_in      = T_in
        self.T_total   = T_in + 1
        self.n_samples = n_samples
        self.num_nodes = num_nodes
        self.n_steps   = diffusion_steps

        self.scheduler = DDPMScheduler(diffusion_steps)
        self.denoiser  = diff_CSDI(
            num_nodes    = num_nodes,
            T_total      = self.T_total,
            channels     = channels,
            n_layers     = n_layers,
            nheads       = nheads,
            diffusion_dim = 128,
            side_dim     = 128,
            inputdim     = 2,
        )

    def to(self, device):
        super().to(device)
        self.scheduler.to(device)
        return self

    def compute_loss(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        x : [B, T_in, N, F]
        y : [B, N, F]
        """
        B      = x.shape[0]
        device = x.device

        y_0_val = y[..., 0]                                # [B, N]
        t_step  = torch.randint(0, self.n_steps, (B,), device=device)
        noise   = torch.randn_like(y_0_val)                # [B, N]

        # 前向加噪（只对目标步）
        y_t = self.scheduler.q_sample(
            y_0_val.unsqueeze(-1), t_step, noise.unsqueeze(-1)
        ).squeeze(-1)                                      # [B, N]

        # 构造完整序列 [B, N, T_total]
        x_c   = x[..., 0].permute(0, 2, 1)               # [B, N, T_in]
        x_full = torch.cat([x_c, y_t.unsqueeze(-1)], dim=-1)   # [B, N, T_total]

        cond_mask = torch.zeros(B, self.num_nodes, self.T_total, device=device)
        cond_mask[:, :, :self.T_in] = 1.0

        eps_pred_full = self.denoiser(x_full, cond_mask, t_step)  # [B, N, T_total]
        eps_pred      = eps_pred_full[:, :, self.T_in]             # [B, N]
        return F.mse_loss(eps_pred, noise)

    @torch.no_grad()
    def sample(self, x: torch.Tensor) -> torch.Tensor:
        """DDPM 逆向采样，返回 [B, N, 1]（n_samples 次均值）"""
        B, T, N, F = x.shape
        device = x.device
        x_c = x[..., 0].permute(0, 2, 1)    # [B, N, T_in]

        preds = []
        for _ in range(self.n_samples):
            y_t = torch.randn(B, N, device=device)   # [B, N]

            for step in reversed(range(self.n_steps)):
                t_batch = torch.full((B,), step, device=device, dtype=torch.long)

                x_full    = torch.cat([x_c, y_t.unsqueeze(-1)], dim=-1)  # [B, N, T_total]
                cond_mask = torch.zeros(B, N, self.T_total, device=device)
                cond_mask[:, :, :self.T_in] = 1.0

                eps_full = self.denoiser(x_full, cond_mask, t_batch)  # [B, N, T_total]
                eps      = eps_full[:, :, self.T_in]                   # [B, N]

                mu, var = self.scheduler.p_mean_variance(
                    y_t.unsqueeze(-1), t_batch, eps.unsqueeze(-1))
                mu  = mu.squeeze(-1)    # [B, N]
                var = var.squeeze(-1)   # [B, 1]（广播到 [B, N]）

                if step > 0:
                    y_t = mu + var.sqrt() * torch.randn_like(mu)
                else:
                    y_t = mu

            preds.append(y_t)

        return torch.stack(preds, dim=0).mean(dim=0).unsqueeze(-1)  # [B, N, 1]

    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        推理接口，对齐 utils.train_model。
        x       : [B, T_in, N, F]
        returns : [B, N, 1]
        """
        return self.sample(x)
