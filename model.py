...
#   L_rank: -log det(U^T U + εI) / rank_r  (低秩正则，已归一化)
# """
...
#         # 用可学习缩放因子替代 LayerNorm：保留输入量纲/分布，只做轻量梯度稳定。
#         # LayerNorm 会把每帧强制归一化到均值0方差1，破坏原始归一化域的分布，
#         # 导致 val 评估时"归一化域"含义漂移（val MAE 从 ~0.09 变成 ~0.75）。
...
#         L_rank = -log(det(U^T @ U + ε·I)) / rank_r
#     （已除以 rank_r 归一化，lambda_rank 的选取与 rank_r 解耦）
#     """
...
#         归一化说明：
#           原始 logdet = log det(U^T U + εI) 的量级随 rank_r 线性增长
#           （每个对角元贡献约 log(||u_k||^2)，多列叠加后绝对值可达 30-50）。
#           除以 rank_r 将其归一化到 O(1) 量级，使 lambda_rank 的选取与
#           rank_r 和 n_nodes 解耦，无需随配置重新调参。
#         """
...
#         # 除以 rank_r 归一化，使输出稳定在 O(1)，与 lambda_rank=0.1 配合使用
...
#         # ── 频率分流 ──────────────────────────────────────────────────────
#         X_low, X_high = self.decomposer(x)       # [B,T,N,Fin] × 2，可学习缩放（非 LayerNorm）
...
#             nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
#             nn.Linear(hidden_dim, y_dim),
#             # 不加 Tanh：Tanh 把 logvar 限制在 (-1,1)，导致方差估计能力极弱，
#             # variational_loss 卡死。去掉后配合 _log_prob 中的 clamp(-6,4) 即可。
#         )
...
#       lambda_rank      : 秩正则权重（默认 0.1，rank_loss 已/rank_r 归一化）
...
#         lambda_rank=0.1,    # 已/rank_r 归一化，与 ModelConfig 默认值保持一致
#         rank_r=8,
...
# """
# GridCFN-DMSD (Optimized Version): Dual-track Multi-Scale Disentanglement
# （在多步预测版基础上引入 DMSD 架构改进）

# 优化改进说明：
#   1. Guided GCN：将数据加载器构建的先验相关性邻接矩阵 `adj_norm` 通过可学习的 alpha 
#      与自适应学习的图结构进行融合，避免图拓扑从头学习导致的过拟合。
#   2. Scale Restoration：在 GCN 后置 LayerNorm 稳定尺度的同时，重新乘上频域分流器
#      的 learnable scale（取 mean() 标量），使模型重新获得分配趋势与残差能量级的能力。
# """
...
#         # ─── [修改] 新增：可学习融合系数，控制自适应图与先验图的混合 ───
#         self.alpha = nn.Parameter(torch.tensor(0.0))  # 经过 sigmoid 后初始化为 0.5 左右
...
#         A_adaptive = self.U @ self.U.T / math.sqrt(self.rank_r)  # [N, N]
#         A_adaptive = F.softmax(A_adaptive, dim=-1)
        
#         if adj_norm is not None:
#             # 约束 alpha 在 [0, 1] 之间
#             alpha = torch.sigmoid(self.alpha)
#             # return (1.0 - alpha) * A_adaptive + alpha * adj_norm.to(A_adaptive.device)
...
# # ---------------------------------------------------------------------------
# # 6. DualTrackBackbone — 双轨 Backbone
# # ---------------------------------------------------------------------------
...
#     # ─── [修改] 接口新增 adj_norm 参数，用于引导两轨图卷积 ───
#     def forward(self, x: torch.Tensor, adj_norm: torch.Tensor = None):
#         B, T, N, Fin = x.shape

#         X_low, X_high = self.decomposer(x)       # [B,T,N,Fin] × 2

#         X_low_pooled  = self.proj_low(X_low.mean(dim=1))    # [B, N, env_dim]
#         X_high_pooled = self.proj_high(X_high.mean(dim=1))  # [B, N, stoch_dim]

#         # 环境支路：LowRankGCN -> LayerNorm -> TCN_e -> proj -> Pool
#         # [修改] 传入先验图进行引导
#         e_gcn = self.gcn_e(
#             X_low.reshape(B * T, N, Fin), adj_norm=adj_norm
#         ).reshape(B, T, N, -1)                              # [B,T,N,env_dim]
        
#         # ─── [修改] 重点：归一化之后，再乘上频域分流器的 scale，重构能量量纲 ───
#         e_gcn = self.norm_gcn_e(e_gcn) * self.decomposer.scale_low.mean()
        
#         He_seq_raw = self.tcn_e(e_gcn.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)
#         He_seq = self.proj_tcn_e(He_seq_raw)
#         He = self.pool_e(He_seq)

#         # 因果支路：SparseGCN -> LayerNorm -> TCN_s -> proj -> Pool
#         # [修改] 传入先验图进行引导
#         s_gcn = self.gcn_s(
...
#         # ─── [修改] 重点：归一化之后，再乘上频域分流器的 scale，重构能量量纲 ───
#         s_gcn = self.norm_gcn_s(s_gcn) * self.decomposer.scale_high.mean()
...
# # ---------------------------------------------------------------------------
# # 8. MultiScaleContext — 多尺度时间上下文提取
# # ---------------------------------------------------------------------------
...
#         self.club_e = CLUBEstimator(env_dim,   stoch_dim)
#         self.club_s = CLUBEstimator(stoch_dim, env_dim)
...
# ---------------------------------------------------------------------------
# 6. DualTrackBackbone — 双轨 Backbone (优化尺度还原)
# ---------------------------------------------------------------------------
...
class DualTrackBackbone(nn.Module):
    def __init__(self, n_nodes: int, in_dim: int,
        ...
        self.gcn_e   = LowRankGCN(n_nodes, in_dim, gcn_hidden, env_dim,
                                   rank_r=rank_r, n_layers=gcn_layers)
        # ─── [修改] 重点：移除对空间尺度分布有平抑破坏作用的 LayerNorm，保留多传感器量纲异质性 ───
        self.tcn_e   = TCN(in_dim=env_dim, hidden_dim=tcn_hidden, n_layers=tcn_layers)
        self.proj_tcn_e = nn.Linear(tcn_hidden, env_dim) if tcn_hidden != env_dim else nn.Identity()
        self.pool_e  = AttentionPool(env_dim)
    ...
    def forward(self, x: torch.Tensor, adj_norm: torch.Tensor = None):
        ...
        X_low_pooled  = self.proj_low(X_low.mean(dim=1))
        X_high_pooled = self.proj_high(X_high.mean(dim=1))

        # 环境支路：LowRankGCN -> TCN_e -> Proj -> Pool
        e_gcn = self.gcn_e(
            X_low.reshape(B * T, N, Fin), adj_norm=adj_norm
        ).reshape(B, T, N, -1)
        ...
        # 因果支路：SparseGCN -> TCN_s -> Proj -> Pool
        s_gcn = self.gcn_s(
            X_high.reshape(B * T, N, Fin), adj_norm=adj_norm, wind_mask=self.wind_mask
        ).reshape(B, T, N, -1)
