...
#         # ── Step 1：更新 CLUB 变分网络（内循环）──────────────────────────
#         # [修复1] 全程用 detach 缓存。
#         # 原版在内循环后做了第 2 次 forward，导致 warmup 结束时显存翻倍 OOM。
#         # He_d/Hs_d/X_low_d/X_high_d 在 Step2 的 MI 计算中也复用，
#         # 避免把 backbone 的完整计算图拖入 club MLP 的 forward。
#         He_d      = He.detach()
#         Hs_d      = Hs.detach()
...
#                 var_e    = model.club_e.variational_loss(He_d, X_low_d)
#                 var_s    = model.club_s.variational_loss(Hs_d, X_high_d)
...
#         if train_club:
#             # [修复2] MI 惩罚用 detach 缓存传入 club MLP，
#             # 彻底切断"backbone 计算图通过 club MLP 进入 loss"的路径。
#             # club 参数已通过 Step1 更新，此处只需它输出一个惩罚标量乘以 lambda，
#             # 梯度只需流回 He/Hs → backbone，不需要流过 club MLP 内部权重。
#             # 注：_set_requires_grad 冻结 club 参数，但不截断对 He_d 输入的梯度，
#             # 而 He_d 本身已 detach，所以此处直接用 He_d 完全等价且更节省显存。
#             with torch.no_grad():
#                 mi_e   = model.club_e(He_d, X_low_d)
#                 mi_s   = model.club_s(Hs_d, X_high_d)
#                 mi_val = (mi_e + mi_s).item()

#             # MI 惩罚以标量形式加入 loss，梯度通过 cfm_loss 路径传回 backbone
#             # （club 只提供"当前 MI 有多大"的信号，惩罚强度由 lambda_club 控制）
#             mi_penalty = max(mi_val, 0.0)
#             loss = (cfm_l
...
#         # [修复3] 每 batch 结束后显式释放 GPU 张量，防止多 batch 显存累积
...
#         list(model.club_e.parameters()) + list(model.club_s.parameters()),
#         lr=cfg_train.lr * 2.0,   # 原 0.5x：CLUB 网络需快速跟上主网络，lr 太低跟不上
#         weight_decay=cfg_train.weight_decay,
#     )
...
#     club_inner_steps = getattr(cfg_train, "club_inner_steps",     5)  # 原 3，提高到 5
...
#         # club_optimizer 与主网络保持 2× 比例，随 scheduler 同步衰减
...
# 改进说明：
#   1. model.forward 返回 6 个特征。
#   2. [修改] 交叉解耦变分计算：
#      - club_e: 输入 He_d 与 X_high_d
#      - club_s: 输入 Hs_d 与 X_low_d
#   3. [修改] 引入训练期间 Val 验证加速机制：
#      - Val 的 `n_samples_val` 与 `n_steps_val` 统一缩减为 10，解决计算瓶颈。
# """
...
#             for _ in range(club_inner_steps):
#                 # ─── [修改] 交叉解耦变分参数 ───
#                 var_e    = model.club_e.variational_loss(He_d, X_high_d) # He 与高频残差
#                 var_s    = model.club_s.variational_loss(Hs_d, X_low_d)  # Hs 与低频趋势
...
#             # ─── [修改] 交叉解耦惩罚项计算 ───
#             with torch.no_grad():
#                 mi_e   = model.club_e(He_d, X_high_d) # He 与高频残差的 MI
#                 mi_s   = model.club_s(Hs_d, X_low_d)  # Hs 与低频趋势的 MI
#                 mi_val = (mi_e + mi_s).item()
...
#     # ─── [修改] 极大地压缩训练期间的验证开销，默认限制在 10 采样 & 10 步 ───
...
#         # ─── [修改] 传递轻量化参数（n_samples_val 和 n_steps_val） ───
#         val_m = evaluate(
#             model, val_loader, adj_norm, edge_index, device,
#             scaler=scaler,
